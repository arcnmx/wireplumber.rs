...
impl SpaPodParser {
    ...
    pub fn new_object(pod: &SpaPod, id_name: &str) -> SpaPodParser {
        unsafe {
            from_glib_full(ffi::wp_spa_pod_parser_new_object(pod.to_glib_none().0, &mut (id_name.as_ptr() as *const libc::c_char)))
        }
    }
