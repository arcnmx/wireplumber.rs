...
impl SpaIdTable {
	pub fn from_name(name: &str) -> Option<Self> {
		unsafe {
			from_glib(ffi::wp_spa_id_table_from_name(name.to_glib_none().0))
		}
	}

	pub fn new_iterator(&self) -> crate::Iterator {
		unsafe {
			from_glib_full(ffi::wp_spa_id_table_new_iterator(self.into_glib()))
		}
	}
	...
	pub fn find_value(&self, value: u32) -> Option<SpaIdValue> {
		unsafe {
			from_glib(ffi::wp_spa_id_table_find_value(self.into_glib(), value))
		}
	}

	pub fn find_value_from_name(&self, name: &str) -> Option<SpaIdValue> {
		unsafe {
			from_glib(ffi::wp_spa_id_table_find_value_from_name(self.into_glib(), name.to_glib_none().0))
		}
	}

	pub fn find_value_from_short_name(&self, name: &str) -> Option<SpaIdValue> {
		unsafe {
			from_glib(ffi::wp_spa_id_table_find_value_from_short_name(self.into_glib(), name.to_glib_none().0))
		}
	}
}
...
impl StaticType for SpaIdTable {
	fn static_type() -> Type {
		unsafe {
			from_glib(ffi::wp_spa_id_table_get_type())
		}
	}
}
