...
use crate::plugin::{ComponentLoader, PluginImpl};
use crate::error;
use crate::prelude::*;
...
impl<T: ComponentLoaderImpl> ComponentLoaderImplExt for T {
	...
	fn parent_supports_type(&self, loader: &Self::Type, type_: String) -> bool {
		...
		let f = parent.supports_type.expect("No parent class implementation for \"supports_type\"");
		unsafe {
			from_glib(
				f(loader.unsafe_cast_ref::<ComponentLoader>().to_glib_none().0, type_.to_glib_none().0)
			)
		}
	}
	...
	fn parent_load(&self, loader: &Self::Type, component: String, type_: String, args: Option<Variant>) -> Result<(), Error> {
		...
		unsafe {
			...
			let res = from_glib(
				f(loader.unsafe_cast_ref::<ComponentLoader>().to_glib_none().0, component.to_glib_none().0, type_.to_glib_none().0, args.to_glib_none().0, &mut error)
			);
...
unsafe impl<T: ComponentLoaderImpl + PluginImpl> IsSubclassable<T> for ComponentLoader {
	fn class_init(class: &mut glib::Class<Self>) {
		...
		unsafe extern "C" fn supports_type<T: ComponentLoaderImpl>(loader: *mut ffi::WpComponentLoader, type_: *const libc::c_char) -> glib::ffi::gboolean {
			let this = &*(loader as *mut T::Instance);
			let this = this.imp();
			let loader: Borrowed<ComponentLoader> = from_glib_borrow(loader);

			this.supports_type(loader.unsafe_cast_ref(), from_glib_none(type_))
				.into_glib()
		}

		unsafe extern "C" fn load<T: ComponentLoaderImpl>(loader: *mut ffi::WpComponentLoader, component: *const libc::c_char, type_: *const libc::c_char, args: *mut glib::ffi::GVariant, error: *mut *mut glib::ffi::GError) -> glib::ffi::gboolean {
			let this = &*(loader as *mut T::Instance);
			let this = this.imp();
			let loader: Borrowed<ComponentLoader> = from_glib_borrow(loader);

			match this.load(loader.unsafe_cast_ref(), from_glib_none(component), from_glib_none(type_), from_glib_none(args)) {
				Ok(()) => true,
				Err(e) => {
